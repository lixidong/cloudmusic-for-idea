...
- 预缓冲播放：点击播放后先下载前 512KB，然后边下载边播放，显著降低首秒延迟
- 分级恢复机制：播放卡住时自动尝试 3 级恢复（重试 → 重建解码器 → 重新下载），仅连续失败才切歌
- 断点续传：未下载完成的歌曲再次播放时从断点继续，无需重新下载
- 播放状态机：支持 IDLE/PLAYING/PAUSED/BUFFERING/RECOVERING/FAILED 六种状态，UI 可展示缓冲中和恢复中提示

### Changed

- 下载与播放解耦：独立 StreamDownloader 负责下载（带 30 秒读取超时），AudioEngine 播放本地文件，消除网络抖动对播放的直接影响
- MediaCache 增强：支持 `.mp3.partial` 文件，未完成的下载保留而非删除
- RecoveryController 替代原有简单超时切歌逻辑，提供更智能的卡顿处理

### Fixed

- 播放卡顿/卡死：通过预缓冲、分级恢复、下载超时等多层机制，彻底解决网络不稳定导致的播放中断
